...
def request():
    ...
    try:
        for url in urls:
            ...
            nr_line = nr_line
            url = url.replace('\n', '')
            req = requests.head('https://' + url, timeout=7)
            print (t, w+'['+str(nr_line)+']' +r+ '['+w+ str(req.url) +r+']'+w+ '\r\t\t\t\t\t|  RespTime: ' + str(req.elapsed.total_seconds()) + '\r\t\t\t\t\t\t\t\t|  --<{' + str(req.status_code) + '}')
            #check alive:
            if req.status_code == 200:# can do more here if u want [...]
                list.append(req.url)
                continue
    ...
    for domains in list:
        ...
        print(domains)
    print('\n\r'+f, r + '['+w+str(error)+r+']', w + 'Error was discovered.\n\r'+f, r + '['+w+str(timeouts)+r+']', w + 'Timeouts was discovered.')
